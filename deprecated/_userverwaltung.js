document.addEventListener("DOMContentLoaded", () => {
    ...
    async function showUserDetails(userId) {
        try {
            ...
            const userRolesMap = await api.getRoleMap();
            ...
            const tmpBtn = document.getElementById("tmp-rights-action-btn");

            tmpBtn.onclick = () => openTmpRightsOverlay(user);
    ...
    function openTmpRightsOverlay(user){

        renderTmpRightsForm(user);
        tmpRightsOverlay.classList.add("active");
    }

    function renderTmpRightsForm(user){

        tmpRightsModal.innerHTML = `
        ...
        document
            .getElementById("tmp-rights-close-btn")
            .onclick = closeTmpRightsOverlay;

        loadRoleOptions();
        bindTmpRightsSubmit(user);
    }
...
function bindTmpRightsSubmit(user){

    document
        .getElementById("tmp-rights-submit")
        .onclick = async () => {

            const role = document.getElementById("tmp-role-select").value;
            ...
            try{
                ...
                const res = await fetch("/api/processes/temp-role", {
                    method:"POST",
                    headers:{ "Content-Type":"application/json"},
                    body: JSON.stringify({
                ...
                showFlash("Temporäre Rolle beantragt!", "success");
                closeTmpRightsOverlay();

            }catch(err){
                console.error(err);
                showFlash("Fehler beim Starten des Prozesses.", "failure");
            }
        };
}
...
async function loadRoleOptions(){
    ...
    const roles = [
        {id: 1, name: "SAP Admin"},
        {id: 2, name: "HR Zugriff"},
        {id: 3, name: "Finance Read"},
        {id: 4, name: "Domain Admin"}
    ];
